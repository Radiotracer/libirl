	...
	*/
{
	...
	int sMaxOffs, sNumOffs, tmpMaxDepthIndex, iSliceP, iColP, PixelValue,
		iDepth, iOffs;
	float sumscat, sumtot;
	Drf_t *drf;
	...
	if (!bActImgRotated){
		vPrintMsg(8, 
			"Project: rotating activity image for angle %d\n",iAngle);
		if (bFastRotate){
		...
		vPrintMsg(8,
			"Project: skipping activity image rotation for angle %d\n",
			iAngle);
	}
#ifdef DEBUG
	...
	*/
	...
	for (iRow=0; iRow < iNumRotPixs; iRow++) {
		...
		pfProjPixels = &(pfRotPixels[iRow*iNumSlices*iNumRotPixs]);
			/*pfPrjPixels will contain a pointer to the pixels to be projected. It
		...
		/*add_scat is a function that uses the old scatter model to add scatter 
				to the projections.sTempImages.ScatterEstimate is a matrix that 
				contains the sum of scatter and the current image estimate, 
				so update the iamge pointer to point to it*/
		...
		if(bModelDrf){
			if (fDist > 0.0){/* for fdistance < don't do any convolution*/
				ConvolveDrf(pfProjPixels,iNumRotPixs, iNumSlices, 
					&drf[iDist],
		...
		if(bModelAtn)
			for (i=0; i < iNumSlices * iNumRotPixs; i++) 
				calcprj[i] += pfProjPixels[i] *
									exp(-pfSumAtnTable[i]*fAtnFac)*fScaleFac;
		else
			for (i=0; i < iNumSlices * iNumRotPixs; i++)
	...
	for(iRow=0; iRow < iNumRotPixs; iRow++){
		...
		if(bModelDrf){
			if (fDist > 0.0){/* only do convolution for distances > 0*/
				ConvolveDrf(pfBackProjPixels,iNumRotPixs, iNumSlices, 
					&drf[iDist],
					psDrfTab->bFFTconvolve,
               sTempImages.ConvImage);
		...
		/*pfBackProjPixels always points to the array to be summed into the 
			backprojection image. Simply copy it, multiplying by the attenuation 
			factor if necessary*/
		if(bModelAtn){
			for (i=0; i<iNumSlices*iNumRotPixs; i++){ 
				rotreconmatrix[_23dptr(iRow, i)] = pfBackProjPixels[i] * 
				exp(-pfSumAtnTable[i]*fAtnFac)*fScaleFac;
			}
		} else
			for (i=0; i< iNumSlices*iNumRotPixs; i++)
