/*
	...
	$Id: convolve_ypad.c 58 2006-01-30 21:06:01Z mjs $
...
void ConvolveDrfYpad(float *pfIn, /* input 2d slice */
					int inX, int inY, /* size of input slice */
					Drf_t *psDrf, /* Drf to use in convolution */
					int bFFTConvolve,
					int iYpad,// number of rows above and below image to pad
					int iYavg,// number of rows to average. The averaged value
					          // for each column is copied into the padded region
...

{
	...
	*/
	...
	if(!bFFTConvolve){
		conv2d_ypad(pfIn, inX, inY, pfPsf, iPsfX-1, iPsfY-1, iYpad, iYavg, pfOut);
...
int main(int argc, char **argv)
{
	...
	if (--argc != 8){
	fprintf(stderr,"usage: convolve in1.im in2.im im.out flag1 flag2 ypad yavg outpsf\n");
	...
	ConvolveDrfYpad(in1,xdim1,ydim1,&Drf,bFFTconv, iYpad, iYavg, outvec);
